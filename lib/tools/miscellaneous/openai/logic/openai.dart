...
class OPENAIgetChatJobData {
  ...
  final String openai_language;
  final OPENAI_TASK openai_task;
  final GCWFile openai_audiofile;

  OPENAIgetChatJobData(
      {required this.openai_audiofile,
      required this.openai_speed,
      required this.openai_voice,
      required this.openai_api_key,
      required this.openai_model,
      required this.openai_prompt,
      required this.openai_temperature,
      required this.openai_image_size,
      required this.openai_image_url,
      required this.openai_language,
...
Future<OpenAItaskOutput> OpenAIrunTaskAsync(GCWAsyncExecuterParameters? jobData) async {
  if (jobData?.parameters is! OPENAIgetChatJobData) {
    return Future.value(OpenAItaskOutput(
  ...
  OpenAItaskOutput output = OpenAItaskOutput(
      status: OPENAI_TASK_STATUS.ERROR,
      httpCode: '',
      httpMessage: '',
      textData: '',
  ...
  switch (ChatGPTgetChatJob.openai_task) {
    case OPENAI_TASK.CHAT:
      output = await _OpenAIgetTextAsync(ChatGPTgetChatJob.openai_api_key, ChatGPTgetChatJob.openai_model,
          ChatGPTgetChatJob.openai_prompt, ChatGPTgetChatJob.openai_temperature,
    ...
    case OPENAI_TASK.AUDIO_TRANSLATE:
      output = await _OpenAIgetAudioAsync(ChatGPTgetChatJob.openai_api_key, ChatGPTgetChatJob.openai_prompt,
          ChatGPTgetChatJob.openai_temperature, ChatGPTgetChatJob.openai_audiofile, ChatGPTgetChatJob.openai_task,
          sendAsyncPort: jobData.sendAsyncPort);
      break;
    case OPENAI_TASK.SPEECH:
      output = await _OpenAIgetSpeechAsync(ChatGPTgetChatJob.openai_api_key, ChatGPTgetChatJob.openai_model,
