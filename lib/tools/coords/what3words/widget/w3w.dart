...
import 'package:gc_wizard/tools/coords/what3words/logic/w3w.dart';
...
class FormatConverterW3WState extends State<FormatConverterW3W> {
  ...
  CoordinateFormatKey _currentLanguage = CoordinateFormatKey.WHAT3WORDS_DE;
  ...
  Widget _buildEncode() {
    return Column(children: <Widget>[
      ...
      GCWTextDivider(
          text: i18n(context, 'coords_formatconverter_w3w_language'),
      ),
      GCWDropDown<CoordinateFormatKey>(
        value: _currentLanguage,
        items: WHAT3WORDS_LANGUAGE.entries.map((mode) {
      ...
      child: GCWColumnedMultilineOutput(
        data: [
          [
            ...
            formatCoordOutput(
                LatLng(_currentW3wToCoordinates.square_ne.latitude, _currentW3wToCoordinates.square_ne.longitude),
          ...
          [
            ...
            formatCoordOutput(
                LatLng(_currentW3wToCoordinates.square_sw.latitude, _currentW3wToCoordinates.square_sw.longitude),
                defaultCoordinateFormat,
                defaultEllipsoid)
          ],
  ...
  void _calculateOutput(BuildContext context) {
    if (_currentMode == GCWSwitchPosition.right) { // decrypt - W3W to ???
      if (_currentOutputFormat.type == CoordinateFormatKey.ALL) {
