...
Future<Uint8List> _ogaToPCM(Uint8List oggBytes) async {
...
Future<AudioInfo> _ogaAudioInfo(Uint8List bytes) async {

  Uint8List amplitudesData = Uint8List.fromList([]);

  await _ogaToPCM(bytes).then((value) {
    amplitudesData = value;
  });

  final info = await AudioDecoder.getAudioInfoBytes(bytes, formatHint: 'oga');
  return AudioInfo(
