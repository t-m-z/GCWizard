...
enum Op {
  ...
  final OpcodeFormat format;

  const Op(this.format);

  String codePointToString(int codepoint, CodeExtract ex) {
    switch (format) {
      case OpcodeFormat.A:
        return '$name ${ex.extract_A(codepoint)}';
      case OpcodeFormat.A_B:
        return '$name ${ex.extract_A(codepoint)} ${ex.extract_B(codepoint)}';
      case OpcodeFormat.A_C:
        return '$name ${ex.extract_A(codepoint)} ${ex.extract_C(codepoint)}';
      case OpcodeFormat.A_B_C:
        return '$name ${ex.extract_A(codepoint)} ${ex.extract_B(codepoint)} ${ex.extract_C(codepoint)}';
      case OpcodeFormat.A_Bx:
        return '$name ${ex.extract_A(codepoint)} ${ex.extract_Bx(codepoint)}';
      case OpcodeFormat.A_sBx:
        return '$name ${ex.extract_A(codepoint)} ${ex.extract_sBx(codepoint)}';
      case OpcodeFormat.Ax:
        return '$name <Ax>';
      case OpcodeFormat.sBx:
        return '$name ${ex.extract_sBx(codepoint)}';
      default:
        return name;
    }
  }
