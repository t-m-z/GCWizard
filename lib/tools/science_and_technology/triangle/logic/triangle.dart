...
XYPoint TriLinearToXYPoint(TriLinearPoint P, XYPoint A, XYPoint B, XYPoint C) {
  ...
  return XYPoint(x: A.x + lc * c1 - k * P.z * c2, y: A.y + lc * c2 + k * P.z * c1);
}
...
XYPoint intersectVectors(XYLine L1, XYLine L2) {
  ...
  try {
    double m = (L1.P1.y * L2.P2.x - L2.P1.y * L2.P2.x - L1.P1.x * L2.P2.y + L2.P1.x * L2.P2.y) /
        (L1.P2.x * L2.P2.y - L1.P2.y * L2.P2.x);

    return XYPoint(x: L1.P1.x + m * L1.P2.x, y: L1.P1.y + m * L1.P2.y);
  } catch (e) {
    return XYPoint(x: 0, y: 0);
  }
}
...
XYPoint triangleGergonne(XYPoint A, XYPoint B, XYPoint C) {
  ...
  XYPoint X = intersectVectors(XYLine(P1: B, P2: _vectorAB(B, C)), XYLine(P1: M, P2: _vectorNorm(_vectorAB(B, C))));
  XYPoint Z = intersectVectors(XYLine(P1: A, P2: _vectorAB(A, B)), XYLine(P1: M, P2: _vectorNorm(_vectorAB(A, B))));
  return intersectVectors(
...
XYPoint triangleNagel(XYPoint A, XYPoint B, XYPoint C) {
  ...
  XYPoint N = intersectVectors(XYLine(P1: A, P2: BexA), XYLine(P1: B, P2: BexB));
  return N;
}
...
List<XYPoint> triangleSidesMidPoints(
  XYPoint A,
  XYPoint B,
  XYPoint C,
) {
...
List<XYPoint> triangleAnglesMidPoints(
  XYPoint A,
  XYPoint B,
  XYPoint C,
) {
...
List<XYPoint> triangleSymmediandPoints(
  XYPoint A,
  XYPoint B,
  XYPoint C,
) {
...
List<XYPoint> triangleAltitudesBasePoints(
  XYPoint A,
  XYPoint B,
  XYPoint C,
) {
  ...
  result.add(intersectVectors(XYLine(P1: B, P2: _vectorDiv(_vectorAB(B, C), _vectorLength(_vectorAB(B, C)))),
      XYLine(P1: A, P2: _vectorDiv(_vectorAB(A, H), _vectorLength(_vectorAB(A, H))))));
  result.add(intersectVectors(XYLine(P1: A, P2: _vectorDiv(_vectorAB(A, C), _vectorLength(_vectorAB(A, C)))),
      XYLine(P1: B, P2: _vectorDiv(_vectorAB(B, H), _vectorLength(_vectorAB(B, H))))));
  result.add(intersectVectors(XYLine(P1: A, P2: _vectorDiv(_vectorAB(A, B), _vectorLength(_vectorAB(A, B)))),
      XYLine(P1: C, P2: _vectorDiv(_vectorAB(C, H), _vectorLength(_vectorAB(C, H))))));
  return result;
}

XYCircle triangleInCircle(
  XYPoint A,
  XYPoint B,
  XYPoint C,
) {
  ...
  XYPoint S = intersectVectors(
      XYLine(
          P1: A,
          P2: _vectorAdd(_vectorDiv(_vectorAB(A, B), _vectorLength(_vectorAB(A, B))),
              _vectorDiv(_vectorAB(A, C), _vectorLength(_vectorAB(A, C))))),
      XYLine(
          P1: B,
          P2: _vectorAdd(_vectorDiv(_vectorAB(B, A), _vectorLength(_vectorAB(B, A))),
              _vectorDiv(_vectorAB(B, C), _vectorLength(_vectorAB(B, C))))));
...
XYCircle triangleCircumCircle(
  ...
  XYPoint S = intersectVectors(
      XYLine(P1: SA, P2: _vectorNorm(_vectorAB(B, C))), XYLine(P1: SB, P2: _vectorNorm(_vectorAB(A, C))));
  return XYCircle(
    x: S.x,
    y: S.y,
    // ru = a/(2·sin(alpha))
...
XYCircle triangleFeuerbachCircle(
  XYPoint A,
  XYPoint B,
  XYPoint C,
) {
...
List<XYCircle> triangleExCircles(
  XYPoint A,
  XYPoint B,
  XYPoint C,
) {
...
Future<Uint8List> triangleData2Image({
  ...
  if (EB.x - EA.r < minX) minX = EB.x - EB.r;
  if (EB.x + EA.r > maxX) maxX = EB.x + EB.r;
  if (EC.x - EA.r < minX) minX = EC.x - EC.r;
  if (EC.x + EA.r > maxX) maxX = EC.x + EC.r;

  if (A.y < minY) minY = A.y;
  if (A.y > maxY) maxY = A.x;
  ...
  canvas.drawLine(Offset(A.x * SCALE + offsetX, A.y * SCALE + offsetY),
      Offset(B.x * SCALE + offsetX, B.y * SCALE + offsetY), paint);
  canvas.drawLine(Offset(B.x * SCALE + offsetX, B.y * SCALE + offsetY),
      Offset(C.x * SCALE + offsetX, C.y * SCALE + offsetY), paint);
  canvas.drawLine(Offset(C.x * SCALE + offsetX, C.y * SCALE + offsetY),
      Offset(A.x * SCALE + offsetX, A.y * SCALE + offsetY), paint);

  // draw Lines
  paint.color = Colors.orangeAccent;
  canvas.drawLine(Offset(A.x * SCALE + offsetX, A.y * SCALE + offsetY),
      Offset(AA.x * SCALE + offsetX, AA.y * SCALE + offsetY), paint);
  canvas.drawLine(Offset(B.x * SCALE + offsetX, B.y * SCALE + offsetY),
      Offset(AB.x * SCALE + offsetX, AB.y * SCALE + offsetY), paint);
  canvas.drawLine(Offset(C.x * SCALE + offsetX, C.y * SCALE + offsetY),
      Offset(AC.x * SCALE + offsetX, AC.y * SCALE + offsetY), paint);
  paint.color = Colors.orange;
  canvas.drawLine(Offset(A.x * SCALE + offsetX, A.y * SCALE + offsetY),
      Offset(MSA.x * SCALE + offsetX, MSA.y * SCALE + offsetY), paint);
  canvas.drawLine(Offset(B.x * SCALE + offsetX, B.y * SCALE + offsetY),
      Offset(MSB.x * SCALE + offsetX, MSB.y * SCALE + offsetY), paint);
  canvas.drawLine(Offset(C.x * SCALE + offsetX, C.y * SCALE + offsetY),
      Offset(MSC.x * SCALE + offsetX, MSC.y * SCALE + offsetY), paint);
