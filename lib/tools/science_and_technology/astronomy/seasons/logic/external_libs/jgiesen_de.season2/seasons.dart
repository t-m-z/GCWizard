...
double _s(double T) {
  ...
  x += 203 * cos(K * (337.23 + 32964.467 * T));
  x += 199 * cos(K * (342.08 + 20.186 * T));
  x += 182 * cos(K * (27.85 + 445267.112 * T));
  x += 156 * cos(K * (73.14 + 45036.886 * T));
  x += 136 * cos(K * (171.52 + 22518.443 * T));
  x += 77 * cos(K * (222.54 + 65928.934 * T));
  x += 74 * cos(K * (296.72 + 3034.906 * T));
  x += 70 * cos(K * (243.58 + 9037.513 * T));
  x += 58 * cos(K * (119.81 + 33718.147 * T));
  x += 52 * cos(K * (297.17 + 150.678 * T));
  x += 50 * cos(K * (21.02 + 2281.226 * T));

  x += 45 * cos(K * (247.54 + 29929.562 * T));
  x += 44 * cos(K * (325.15 + 31555.956 * T));
  x += 29 * cos(K * (60.93 + 4443.417 * T));
  x += 18 * cos(K * (155.12 + 67555.328 * T));

  x += 17 * cos(K * (288.79 + 4562.452 * T));
  x += 16 * cos(K * (198.04 + 62894.029 * T));
  x += 14 * cos(K * (199.76 + 31436.921 * T));
  x += 12 * cos(K * (95.39 + 14577.848 * T));
  x += 12 * cos(K * (287.11 + 31931.756 * T));
  x += 12 * cos(K * (320.81 + 34777.259 * T));
  x += 9 * cos(K * (227.73 + 1222.114 * T));
  x += 8 * cos(K * (15.45 + 16859.074 * T));
